...
# Assuming your data is stored in a DataFrame named employees_jobs_df
data = {
...
employees_jobs_df = pd.DataFrame(data)

# Count the number of jobs for each employee
jobs_count = employees_jobs_df.groupby('EMPLOYEE_ID')['JOB_ID'].count()

# Filter employees with two or more jobs
employees_with_multiple_jobs = jobs_count[jobs_count >= 2].index

# Display the result
result_df = employees_jobs_df[employees_jobs_df['EMPLOYEE_ID'].isin(employees_with_multiple_jobs)]
print(result_df)
